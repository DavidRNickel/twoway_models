...
if __name__=='__main__':
    ...
    outfile = open(os.path.join(conf.save_dir, conf.test_log_file), 'w')
    sys.stdout=outfile
    ...
    mean1_train = torch.zeros(conf.N_channel_use)
    std1_train  = torch.zeros(conf.N_channel_use)
    mean2_train = torch.zeros(conf.N_channel_use)
    std2_train  = torch.zeros(conf.N_channel_use)
    mean1_total = 0
    std1_total = 0
    mean2_total = 0
    std2_total = 0
    ...
    with torch.no_grad():
        for i in range(N_iter):
            bit1   = bit1_train[conf.batch_size*i:conf.batch_size*(i+1),:,:].view(parameter.batch_size, parameter.N_bits,1) 
            bit2   = bit2_train[conf.batch_size*i:conf.batch_size*(i+1),:,:].view(parameter.batch_size, parameter.N_bits,1) 
            noise1 = noise1_train[conf.batch_size*i:conf.batch_size*(i+1),:,:].view(bit1.shape[0], parameter.N_channel_use,1)
            noise2 = noise2_train[conf.batch_size*i:conf.batch_size*(i+1),:,:].view(bit2.shape[0], parameter.N_channel_use,1)
    ...
    ber2_sum  = 0
    bler2_sum = 0
    power1_sum = np.zeros((conf.batch_size, conf.N_channel_use ,1))
    power2_sum = np.zeros((conf.batch_size, conf.N_channel_use ,1))

    pbar = tqdm(total=N_iter)
    for ii in range(N_iter):
        pbar.update(1)
        ber1_tmp, ber2_tmp, bler1_tmp, bler2_tmp, power1_tmp, power2_tmp = test_model(model, conf, N_small)
        ber1_sum += ber1_tmp
        ber2_sum += ber2_tmp
        bler1_sum += bler1_tmp
        bler2_sum += bler2_tmp
        ...
        #     print('Power1: ', round(np.sum(power1_sum)/(conf.batch_size*(ii+1)),3))
        #     print('Power2: ', round(np.sum(power2_sum)/(conf.batch_size*(ii+1)),3))
