...
if __name__=='__main__':
    ...
    use_cuda = conf.use_cuda and torch.cuda.is_available()
    ...
    outfile = open(os.path.join(save_dir, conf.train_log_file), 'w')
    sys.stdout=outfile
    ...
    SNR1 = conf.SNR1               # SNR at User1 in dB
    np1 = 10**(-SNR1/10)   # noise power1 -- Assuming signal power is set to 1
    sigma1 = np.sqrt(np1)
    SNR2 = conf.SNR2               # SNR at User2 in dB
    np2 = 10**(-SNR2/10)
    sigma2 = np.sqrt(np2)

    # Training set: tuples of (stream, noise1, noise 2)
    N_train = int(1e7)  # number of training set
    bit1_train     = torch.randint(0, 2, (N_train, conf.N_bits, 1))
    bit2_train     = torch.randint(0, 2, (N_train, conf.N_bits, 1))
    noise1_train   = sigma1*torch.randn((N_train, conf.N_channel_use, 1)) 
    noise2_train   = sigma2*torch.randn((N_train, conf.N_channel_use, 1)) 
    ...
    if use_cuda:
        model = Twoway_coding(conf).to(device)
        torch.backends.cudnn.benchmark = True
    else:
        model = Twoway_coding(conf)

    model.sigma1 = sigma1
    model.sigma2 = sigma2

    print(model)
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.confs()), lr=conf.learning_rate)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.95)

    # Training
    num_epoch = 100
    clipping_value = conf.grad_clip
    ...
    N_iter = (N_train//conf.batch_size)
    ...
    for epoch in range(num_epoch):
        ...
        for i in range(N_iter):
            bit1 = bit1_train[conf.batch_size*i:conf.batch_size*(i+1),:,:].view(conf.batch_size, conf.N_bits,1) 
            bit2 = bit2_train[conf.batch_size*i:conf.batch_size*(i+1),:,:].view(conf.batch_size, conf.N_bits,1) 
            noise1 = noise1_train[conf.batch_size*i:conf.batch_size*(i+1),:,:].view(conf.batch_size, conf.N_channel_use,1)
            noise2 = noise2_train[conf.batch_size*i:conf.batch_size*(i+1),:,:].view(conf.batch_size, conf.N_channel_use,1)
            ...
            if conf.output_type == 'bit_vector':
                loss = F.binary_cross_entropy(X1_hat, bit1) + F.binary_cross_entropy(X2_hat, bit2)
            elif conf.output_type == 'one_hot_vector':
                bit1_hot =  one_hot(bit1).view(conf.batch_size, 2**conf.N_bits, 1) # (batch,2^K,1)
                bit2_hot =  one_hot(bit2).view(conf.batch_size, 2**conf.N_bits, 1)
            ...
            # training
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.confs(), clipping_value)
        ...
        ber1_val, ber2_val, bler1_val, bler2_val, _, _ = test_model(model, conf, N_validation)
