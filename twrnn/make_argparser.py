...
def make_parser():
    ...
    parser.add_argument('--use-tensorboard', type=lambda x: bool(strtobool(x)), default=True if torch.cuda.is_available() else False, nargs='?', const=True)
    ...
    parser.add_argument('--train-log-file', type=str, default='tmptrain', nargs='?', help='file to which train logs are written', required=True)
    parser.add_argument('--test-log-file', type=str, default='tmptest', nargs='?', help='file to which test logs are written', required=True)
    ...
    # Encoder
    parser.add_argument('--encoder-act-func', type=str, default='tanh', nargs='?', help='', required=True)
    ...
    parser.add_argument('--input-type', type=str, default='bit_vector', nargs='?', help='one of "bit_vector" or "one_hot_vector"', required=True)
    parser.add_argument('--output-type', type=str, default='one_hot_vector', nargs='?', help='one of "bit_vector" or "one_hot_vector"', required=True)
    parser.add_argument('--decoder-info', type=str, default='None', nargs='?', help='one of "bit_estimate", "state_vector", "None" for encoder input', required=True)
    parser.add_argument('--encoder-info', type=str, default='tran_symbol', nargs='?', help='one of "tran_symbol", "state_vector", "None" for decoder input', required=True)
    parser.add_argument('--SNR1', type=float, default=1, help='SNR1') 
    parser.add_argument('--SNR2', type=float, default=20, help='SNR2')


    # Training hyperparameters
    parser.add_argument('--learning-lr', type=float, default=.01, help='optimizer learning rate')
